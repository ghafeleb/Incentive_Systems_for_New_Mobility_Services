function initEta_prestep(iterRun, n_time, n_time_inc_start, n_time_inc_end, ...
    seedData, step_size, region_, setting_region, fileName0, folderName0)
%% Initialization
% step_size = 0.01;
% For deterministic case, percentage of user/nonuser is not important
% bacause both of the behave deterministically
nonuser_perc_prob0 = 100; % It should stay at 100%
iterRun = iterRun+1;
rng(seedData)
...
log_file = fullfile(folderRunDet, strcat('Run_', num2str(iterRun), '_initEta'), 'outputLog_initEta.txt');    
if iterRun==1
    mkdir(folderRunDet);
...
diary(log_file); % Start logging to a file named 'outputLog_initEta.txt'
...
fprintf('Travel time of no incentivization (hours): %.6f\n', tt_obj_NoInc_Det_total_new)

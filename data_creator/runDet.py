...
def main(args):
    ...
    folderDet = 'Det_MultT' + '_sD' + str(seedData) + \
                    '_ss' + str(step_size)

    # folderDet = 'Det_MultT' + '_sD' + str(seedData) + \
    #                 '_nC' + str(nC) + '_f' + fairness + \
    #                 '_percC' + percC + '_percNonU' + str(percNonU) +  \
    #                 '_ss' + str(step_size)

    if initEta!=-1:
        folderRun = os.path.join('Run_'+str(iterRun)+'_initEta')
    else:
        folderRun = os.path.join('Run_'+str(iterRun))

    theta_OD_Estimation_temp = theta_temp
    theta_opt_temp = theta_temp
    if pad_hr:
        pad_str = '_pad'
        start_hour = start_hour - pad_hr  # 5 AM
    else:
        pad_str = ''
        start_hour = start_hour

    analysis_start_time = datetime.time(start_hour, 0, 0)  # !! Starting time = 3 AM # !!
    time_basis = datetime.time(start_hour, 0, 0)  # !! 3 AM
    ...
    start_hour_str, finish_hr_str = hr_str(start_hour, finish_hr, AM_PM)
    ...
    if altered_speed:
        sv_folder = os.path.join('../data', region_, fileNameSetting1, folderDet, folderRun)
        
        spd_data_address = os.path.join(sv_folder, 'sAftAsDet.csv')
        spd_data_raw_Det = pd.read_csv(spd_data_address, header=None, index_col=False)

        count_data_address = os.path.join(sv_folder, 'vAftAsDet.csv')
        ...
        spd_data_Det = dict()
        count_data_Det = dict()
        t = [datetime.time(start_hour + int(iter / n_times_per_hr), iter % n_times_per_hr * interval_t, 00) for iter in range(num_time)]  # Python 3
        time_basis_temp = datetime.time(start_hour, 0, 0)
        ...
        spd_data_address = '../data/speed_volume/Mar2May_2018_' + region_ + '_AVG' + str(
            interval_t) + '_' + start_hour_str + '-' + finish_hr_str + '_with_linkID_pad/my_link_avg_spd_data_AVG' + str(
            interval_t) + 'min_' + start_hour_str + '-' + finish_hr_str + '_' + region_ + pad_str + '.pickle'
        spd_data = pd.read_pickle(spd_data_address)

        count_data_address = '../data/speed_volume/Mar2May_2018_' + region_ + '_AVG' + str(
            interval_t) + '_' + start_hour_str + '-' + finish_hr_str + '_with_linkID_pad/my_link_avg_count_data_AVG' + str(
            interval_t) + 'min_' + start_hour_str + '-' + finish_hr_str + '_' + region_ + pad_str + '.pickle'
    ...
    Parallel(n_jobs=1, temp_folder='temp', max_nbytes='10M')(
        delayed(save_r)(N, spd_data_Det, analysis_start_time, time_interval,
                        tmp_date, link_dict, link_list, link_loc, path_list, 
                       region_, fileNameSetting1, folderDet, folderRun) for tmp_date in date_need_to_finish)
    ...
    # Parallel(n_jobs=1)(delayed(save_p)(N, spd_data_Det, analysis_start_time, time_interval,
    #                                     tmp_date, path_list, OD_paths, region_, fileNameSetting1, folderDet, folderRun,
    #                                     theta_OD_Estimation) for tmp_date in date_need_to_finish)   
    # t_ = (time.time() - start_time) / 60
    # print "save_p: %.2f minutes" % t_

    # start_time = time.time()
    # theta_opt = theta_opt_temp  # !!!!!!!
    # Parallel(n_jobs=1)(delayed(save_p_opt0)(N, spd_data_Det, analysis_start_time, time_interval,
    #                                          tmp_date, path_list_opt, OD_paths_opt, 
    #                                          region_, fileNameSetting1, folderDet, folderRun, theta_opt)
    ...
    for tmp_date in date_need_to_finish:
        Parallel(n_jobs=1)(delayed(save_tt_joblib)(N, spd_data_Det, analysis_start_time, time_interval,
                                            tmp_date, path_list_opt, OD_paths_opt, region_, fileNameSetting1, folderDet, folderRun)
                        for tmp_date in date_need_to_finish)
    t_ = (time.time() - start_time) / 60

...
def main(args):
    ...
    budget = args.b
    seedData = args.sD
    seedAlgorithm = args.sA
    rho = args.r
    # nIterADMM = args.it + args.sA
    nIterADMM = args.it
    nC = args.nC
    fairness = args.f
    # percC = args.percC
    ...
    folderRun = 'cost_MIPGap' + str(MIPGap)
    if nIterADMM < 0:
        if nIterADMM == -1:
            solver_name = "Gurobi"
        elif nIterADMM == -2:
            ...
            solver_name = "GLPK"
        folderRunSolvingAlgo = solver_name + '_new_Det_initAll2_MultT' +\
                                '_b' + str(budget) + \
                                '_sD' + str(seedData) + \
                                '_sS' + str(seedAlgorithm) + \
                                '_VOT' + str(VOT) + \
                                '_nC' + str(nC) + \
                                '_f' + fairness + \
                                '_percNonU' + str(percNonU) + \
                                '_nTIS' + str(nTIS) + \
                                '_nTIE' + str(nTIE) + \
                                '_ss' + str(step_size) + \
                                '_itN' + str(iterRun)
    else:
        folderRunSolvingAlgo = 'Det_initAll2_MultT' + \
                                '_b' + str(budget) + \
                                '_sD' + str(seedData) + \
                                '_sA' + str(seedAlgorithm) + \
                                '_r' + str(rho) + \
                                '_it' + str(nIterADMM) + \
                                '_VOT' + str(VOT) + \
                                '_nC' + str(nC) + \
    ...
    # folderRunSolvingAlgo = 'Det_initAll2_MultT_b' + str(budget) + '_sD' + str(seedData) + \
    #             '_sA' + str(seedAlgorithm) + '_r' + str(rho) + '_it' + str(nIterADMM) + \
    ...
    if pad_hr:
        pad_str = '_pad'
        start_hr = start_hour - pad_hr  # 5 AM
    else:
        pad_str = ''
        start_hr = start_hour
    ...
    if altered_speed:
        sv_folder = os.path.join('../data', region_, fileNameSetting1, folderRunSolvingAlgo, folderRun)
        print("sv_folder:", sv_folder)
    ...
    for tmp_date in date_need_to_finish:
        Parallel(n_jobs=1)(delayed(save_tt_joblib)(N, spd_data_Det, analysis_start_time, time_interval,
                                            tmp_date, path_list_opt, OD_paths_opt, region_, fileNameSetting1, folderRunSolvingAlgo, folderRun)
                        for tmp_date in date_need_to_finish)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--sA',
                        default=2,
                        type=int,
                        help='Seed of the solving algorithm.')
    ...
    parser.add_argument('--it',
                        default=1000,
                        type=int,
                        help='Max # of ADMM iterations.')
    parser.add_argument('--nC',
                        default=1,
                        type=int,
                        help='Number of companies in ADMM.')
    ...
    # parser.add_argument('--folderRunSolvingAlgo',
    #                     default="Det",
    #                     type=str,
